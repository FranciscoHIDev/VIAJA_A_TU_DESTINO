import React from "react";
import NavBar from "../../components/NavBar/NavBar";
import Footer from "../../components/Footer/Footer";

function TermsAndConditions() {
  return (
    <React.Fragment>
      <div className="min-h-screen flex flex-col">
        <header>
          <NavBar />
        </header>
        <main className="flex-grow">
          <div className="my-40 md:mx-20 mx-5 flex flex-col items-center">
            <div>
              <h1 className="text-3xl uppercase text-[#035373] font-bold">
                Términos y condiciones
              </h1>
            </div>
            <div className="mt-10 ">
              <p className="text-[20px] ">
                Los presentes Términos regulan el acceso y el uso del sitio web
                https://viajaatudestino.com/ el cual es propiedad de VIAJA A TU
                DESTINO SAS DE CV., sociedad constituida bajo las leyes
                mexicanas y con domicilio fiscal en Mallorca Norte E-A D-209
                SM260 M73 L2, Privada Talaia, Real Valencia, Cancún , Quintana
                Roo. C.P 77539 (en adelante “VIAJA A TU DESTINO”, la “Página” o
                el “Sitio Web”)”. De este modo, a la página web de VIAJA A TU
                DESTINO, ratifica su acuerdo con los términos y condiciones
                expuestos a continuación. Le pedimos leer cuidadosamente los
                siguientes Términos y Condiciones de Uso antes de hacer uso de
                la Página.
              </p>
              <p className="text-[20px] mt-5">
                Esta página ofrece un sistema de búsqueda de información para
                planear viajes, sin embargo el Sitio Web no gestiona reservas ni
                sistemas de pago de ningún tipo, esto se realiza por medio
                sitios enlazados de terceros, por lo que por estos Términos y
                Condiciones usted reconoce que VIAJA A TU DESTINO no es
                responsable del contenido de la información o anuncios
                publicitarios de terceros que aparecen en su página. En este
                sentido VIAJA A TU DESTINO en ningún momento será responsable de
                reclamos ante la Procuraduría Federal del Consumidor o cualquier
                autoridad administrativa, ni será responsable respecto de
                cualquier acción, demanda, responsabilidad, daños,
                requerimientos, obligaciones, multas, resolución judicial,
                costos y gastos de cualquier naturaleza, incluyendo sin
                limitación, honorarios razonables de abogados y costas legales
                en que pudiera verse involucrado como resultado de los
                contenidos mostrados en su Página.
              </p>
              <p className="text-[20px] mt-5">PROPIEDAD INTELECTUAL</p>
              <p className="text-[20px] mt-5">
                A menos que se acuerde lo contrario por escrito con VIAJA A TU
                DESTINO, ninguna de las disposiciones de estas condiciones le
                otorga al usuario el derecho a utilizar los nombres y las marcas
                de VIAJA A TU DESTINO, sus logotipos, nombres de dominio u otras
                características distintivas de marca. Mediante la utilización de
                esta página, el usuario reconoce y acepta que VIAJA A TU DESTINO
                queda excluida de cualquier responsabilidad que pudiera ser
                causada por el uso no autorizado de las marcas u otros derechos
                de propiedad intelectual de terceros o contenidos en los sitios
                enlazados.
              </p>
              <p className="text-[20px] mt-5">PRIVACIDAD</p>
              <p className="text-[20px] mt-5">
                Su información personal se procesa y almacena en servidores o
                medios magnéticos que mantienen altos estándares de seguridad y
                protección tanto física como tecnológica. “VIAJA A TU DESTINO”,
                con domicilio fiscal en Mallorca Norte E-A D-209 SM260 M73 L2,
                Privada Talaia, Real Valencia, Cancún , Quintana Roo. C.P 77539,
                únicamente utilizará sus datos personales para identificarle,
                localizarle y contactarle para infórmale respecto a nuevas
                promociones y descuentos. Para mayor información acerca del
                tratamiento y de los derechos que pueda hacer valer, accede al
                aviso de privacidad completo a través de nuestro sitio de
                Internet www.viajaatudestino.com
              </p>
              <p className="text-[20px] mt-5">
                MODIFICACIONES DE LOS TÉRMINOS Y CONDICIONES
              </p>
              <p className="text-[20px] mt-5">
                VIAJA A TU DESTINO podrá modificar los Términos y Condiciones en
                cualquier momento haciendo público en su Página de Internet los
                Términos y Condiciones modificados. Todos los Términos y
                Condiciones entrarán en vigor al día siguiente de su
                publicación. Vencido este plazo, se considerará que el cliente
                aceptará los nuevos términos y el contrato continuará vinculando
                a ambas partes.
              </p>
              <p className="text-[20px] mt-5">PROHIBICIONES</p>
              <p className="text-[20px] mt-5">
                Los clientes no podrán manipular sus datos, enviar información
                falsa o utilizar información de manera dolosa respecto a la
                emisión de su comprobante de pago que trasgreda las leyes
                fiscales, de lavado de dinero y demás aplicables en la República
                Mexicana. Este tipo de actividades serán investigadas por VIAJA
                A TU DESTINO podrá rechazar emitir el comprobante de pago y
                consignará el pago, además de realizar las acciones legales a
                que puedan dar lugar a la configuración de delitos penales.
              </p>
              <p className="text-[20px] mt-5">INDEMNIZACIÓN</p>
              <p className="text-[20px] mt-5">
                El Cliente indemnizará y mantendrá a salvo a VIAJA A TU DESTINO,
                a sus filiales, empresas controladas y/o controlantes,
                directivos, administradores, representantes y empleados, por
                cualquier reclamo o demanda por incumplimiento de los Términos y
                Condiciones Generales y demás Políticas que se entienden
                incorporadas al presente o por la violación de cualesquiera
                leyes o derechos de terceros, incluyendo los honorarios de
                abogados que resulten de dicha trasgresión.
              </p>
              <p className="text-[20px] mt-5">LEY APLICABLE Y JURISDICCIÓN</p>
              <p className="text-[20px] mt-5">
                Para todos los efectos de interpretación, cumplimiento o
                ejecución del presente Contrato, las Partes se someten
                expresamente a las leyes aplicables en la Ciudad de Cancún,
                Quintana Roo y se someten a la jurisdicción y competencia de los
                Tribunales de esta Ciudad, renunciando al beneficio de cualquier
                otro fuero o legislación que tengan o llegaren a adquirir en
                virtud de sus domicilios presentes o futuros.
              </p>
            </div>
          </div>
        </main>
        <footer className="mt-auto">
          <Footer />
        </footer>
      </div>
    </React.Fragment>
  );
}

export default TermsAndConditions;
